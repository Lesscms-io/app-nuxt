...
import type { MultilingualValue, LessCMSConfig } from '~/types/api'
...
export function useLanguage(languageOverride?: Ref<string> | string): UseLanguageReturn {
  const config = inject<LessCMSConfig>('lesscms-config', null)
  ...
  const language = computed(() => {
    ...
    return config?.language || runtimeConfig.public.lesscmsDefaultLanguage || 'pl'
  })
